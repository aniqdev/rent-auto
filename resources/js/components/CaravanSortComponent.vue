<template>
    <draggable v-model="caravans" handle=".sort-handler" ghost-class="ghost" @start="drag=true" @end="drag=false" @change="update">
        <transition-group type="transition" :name="!drag ? 'flip-list' : null">
            <div class="card card-custom gutter-b" v-for="caravan in caravans" :key="caravan.id">
                <div class="card-body">
                    <div class="d-flex">
                        <div class="flex-shrink-0 mr-7">
                            <div class="symbol symbol-50 symbol-lg-120 symbol-circle">
                                <img :alt="caravan.name" :src="'../storage/' + caravan.thumbnail">
                            </div>
                        </div>
                        <div class="flex-grow-1">
                            <div class="d-flex align-items-center justify-content-between flex-wrap mt-2">
                                <div class="mr-3">
                                    <a :href="'karavany/' + caravan.id" class="d-flex align-items-center text-dark text-hover-primary font-size-h5 font-weight-bold mr-3">
                                        {{ caravan.name }}
                                    </a>
                                ...
                                <div class="my-lg-0 my-1">
                                    <a :href="'karavany/' + caravan.id + '/edit'" class="btn btn-sm btn-light-primary font-weight-bolder mr-2">Upravit</a>
                                    <form :action="'karavany/' + caravan.id + '/'" method="POST" style="display: inline;">
                                        <input type="hidden" name="_token" :value="csrf">
                                        <input type="hidden" name="_method" value="delete">
                                        <button type="submit" class="btn btn-sm btn-danger font-weight-bolder" onclick="return confirm('Vážně si přejete odstranit tento karavan?')">Smazat</button>
                                    </form>
                                </div>
                            </div>
                            <div class="d-flex align-items-center flex-wrap justify-content-between">
                                <div class="flex-grow-1 font-weight-bold text-dark-50 py-2 py-lg-2 mr-5" v-html="caravan.short_description"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex align-items-center flex-wrap">
                        ...
                        <div class="d-flex align-items-center flex-lg-fill mr-5 my-1">
                            ...
                            <div class="d-flex flex-column text-dark-75">
                                ...
                                <span class="font-weight-bolder font-size-h5">
                                    {{ caravan.reservations.length }}<span class="text-dark-50 font-weight-bold">x</span>
                                </span>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            update() {
                this.caravans.map((caravan, index) => {
                    caravan.sort = index + 1;
                });

                axios.put('/admin/karavany/update/sort', {
