<template>
    <draggable v-model="tabs" handle=".sort-handler" ghost-class="ghost" @start="drag=true" @end="drag=false" @change="update">
        <transition-group type="transition" :name="!drag ? 'flip-list' : null">
            <div class="card card-custom gutter-b" v-for="tab in tabs" :key="tab.id">
                <div class="card-body">
                    <div class="d-flex">
                        <div class="flex-grow-1">
                            <div class="d-flex align-items-center justify-content-between flex-wrap">
                                ...
                                <div class="my-lg-0 my-1">
                                    ...
                                    <a :href="'otazky/' + tab.id + '/edit'" class="btn btn-sm btn-light-primary font-weight-bolder mr-2" v-if="tab.deleted_at == null">Upravit</a>
                                    <form :action="'otazky/' + tab.id + '/'" method="POST" style="display: inline;" v-if="tab.deleted_at == null">
                                        <input type="hidden" name="_token" :value="csrf">
                                        <input type="hidden" name="_method" value="delete">
                                        <button type="submit" class="btn btn-sm btn-danger font-weight-bolder" onclick="return confirm('Vážně si přejete odstranit tuto otázku?')">Smazat</button>
                                    </form>
                                    <a :href="'otazky/' + tab.id + '/restore'" class="btn btn-sm btn-light-primary font-weight-bolder mr-2" v-if="tab.deleted_at !== null">Obnovit</a>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    export default {
        ...
        methods: {
            update() {
                ...
                axios.put('/admin/otazky/update/sort', {
                    tabs: this.tabs
                }).then((response) => {
                    // Success alert
                });
